...
ZIP_FILE_NAME="lambda-sqs-test-$timestamp.zip"
LAYER_FILE_NAME="lambda-sqs-test-layer-$timestamp.zip"
cd ..
...
echo "------------------------------------------------------------------------------"
echo "[Build] lambda_sqs_producer Function"
echo "------------------------------------------------------------------------------"
cd $source_dir

zip -r $build_dist_dir/$ZIP_FILE_NAME ./lambda_sqs_producer/* ./lambda_sqs_consumer/*


echo "------------------------------------------------------------------------------"
echo "[Build] lambda-sqs-test Layer"
echo "------------------------------------------------------------------------------"

DEPENDENCY_EXCLUDE_LIST=${OVERRIDE_DEFAULT_DEPENDENCY_EXCLUDE_LIST:=["aws_cdk","lambda_sqs_test","boto3","dist-info","pip","botocore","jsii","docutils","pkg_resources","setuptools","test","tests","s3transfer"]}
